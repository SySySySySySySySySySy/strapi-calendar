...
const EventScheduleService = ({ strapi }: { strapi: Core.Strapi }) => ({
	getSchedule: async (start: string, end: string, slug: string) => {
		...
		const schedules = await strapi
			.documents(config.collection as any)
			.findMany({
		...
		for (let i = 0; i < diffInDays; i++) {
			...
			const filteredSchedules = schedules.filter((schedule) => {
				return (
					date.isSameOrAfter(schedule.start, "date") &&
					date.isSameOrBefore(schedule.end, "date")
				);
			});
			results[moment(date).format("YYYY-MM-DD")] = filteredSchedules.map(
				(schedule: any) => {
					return {
						name: schedule.name,
